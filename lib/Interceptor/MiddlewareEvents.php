...
namespace FicoEXTScored\Client\Interceptor;
...
use \FicoEXTScored\Client\Interceptor\MyLogger;
use \FicoEXTScored\Client\Model\Errors;
use \FicoEXTScored\Client\Model\Error;

class MiddlewareEvents
{
    
    function __construct(\FicoEXTScored\Client\Interceptor\KeyHandler $signer)
    {
        $this->signer = $signer;
...
function build_error($code, $message){
    $error = new \FicoEXTScored\Client\Model\Error([
        "code" => $code,
        "message" => $message
    ]);
    $errors = new \FicoEXTScored\Client\Model\Errores(["errors" => [$error]]);
